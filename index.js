...
var SIMPLE_PLACEHOLDER_CHAR_REGEX = /\w/;

function isSimplePlaceholderChar(ch) {
    return SIMPLE_PLACEHOLDER_CHAR_REGEX.test(ch);
}
...
exports.createParser = function(listeners, options) {
    ...
    states.INITIAL = Parser.createState({
        enter: function() {
            text = '';
        },
        ...
        char: function(ch, code) {
            ...
            if (code === CODE_LEFT_ANGLE_BRACKET) {
            ...
            } else if (code === CODE_BACK_SLASH) {
                nextCh = parser.lookAtCharCodeAhead(1);
                if (nextCh === CODE_DOLLAR) {
                    // encountered "\$" which is used to escape the $
                    text += '$';
                } else {
                    //
                    text += ch;
                ...
                if (nextCh === CODE_LEFT_CURLY_BRACE) {
                    parser.skip(1);
                    parser.enterState(states.PLACEHOLDER_WITH_BRACES);
                } else {
                    parser.enterState(states.PLACEHOLDER_WITHOUT_BRACES);
                }
            } else {
                text += ch;
            }
    ...
    states.PLACEHOLDER_WITH_BRACES = Parser.createState({
        // We immediately go into the EXPRESSION sub-state to look
        // for the closing "}".
        expression: {
            eof: function() {
                _notifyText(text);
                ...
                // don't include the "${" and "}" in the notification
                _notifyPlaceholder(text.substring(2, text.length - 1));
        ...
        enter: function() {
            text = '$';
            _enterExpressionState('{', CODE_LEFT_CURLY_BRACE, CODE_RIGHT_CURLY_BRACE);
        }
    });

    states.PLACEHOLDER_WITHOUT_BRACES = Parser.createState({
        enter: function() {
            text = '$';
        },

        char: function(ch, code) {
            if (isSimplePlaceholderChar(ch)) {
                text += ch;
            } else {
                _notifyPlaceholder(text.substring(1));
                parser.enterState(states.INITIAL);
                text = ch;
            }
        }
    ...
    states.START_BEGIN_ELEMENT = Parser.createState({
        enter: function() {
            tagName = '';
        },
    ...
    states.ELEMENT_NAME = Parser.createState({
        enter: function() {
    ...
    states.CDATA = Parser.createState({
        enter: function() {
            text = '';
        },
    ...
    states.END_ELEMENT = Parser.createState({
        eof: function() {
    ...
    states.WITHIN_ELEMENT = Parser.createState({
        eof: function() {
    ...
    states.ATTRIBUTE_NAME = Parser.createState({
        eof: function() {
    ...
    states.ATTRIBUTE_VALUE = Parser.createState({
    ...
    // state. The expression delimiters are the following: ({[
    states.EXPRESSION = Parser.createState({
        // We go into the STRING sub-state when we see a single or double
        ...
        char: function(ch, code) {
            ...
            // Are we in an expression that is delimited?
            // If so, stay in the EXPRESSION state until the
            // expression depth reaches zero.
            // Otherwise, look for the ending tag.
            if (expressionEndDelimiter !== undefined) {
    ...
    states.SCRIPT_TAG = Parser.createState({
    ...
    states.STYLE_TAG = Parser.createState({
        // We go into the EXPRESSION sub-state right after entering
        // the STYLE_TAG state.
        // The EXPRESSION state will bubble some events up to the
        // parent state.
    ...
    states.STRING = Parser.createState({
        eof: function() {
            stringHandler.eof();
        },
    ...
    // the STRING state.
    states.STRING_ESCAPE_CHAR = Parser.createState({
        eof: function() {
            stringHandler.eof();
        },
    ...
    states.BLOCK_COMMENT = Parser.createState({
        eof: function() {
            commentHandler.eof();
        },
    ...
    states.LINE_COMMENT = Parser.createState({
        eof: function() {
            commentHandler.eof();
        },
    ...
    states.DECLARATION = Parser.createState({
        enter: function() {
            tagName = '';
        },
    ...
    states.XML_COMMENT = Parser.createState({
        enter: function() {
            comment = '';
        },
