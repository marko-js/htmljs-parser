...
- An html mode attribute value whose lone type argument is cut off by the tag-ending `>`, such as `<let/s=new Set<string>()>` or `fetchJson<User>(url)`, now reports an `AMBIGUOUS_ATTRIBUTE_VALUE` error suggesting `(new Set<string>())`, instead of silently parsing the comparison `new Set<string` with `()>` as body content. It needs a generic callee hugging `<TypeName`, a `(` right after the tag's `>`, a balanced group and then `>` or `/>`, so a real comparison is only caught when the body text starts with a parenthesized group directly followed by `>`.
- The concise mode indentation error now reads "Line indentation does not match indentation of previous line"; it was missing the "not".
